...
* `+proofs/coq/trivial_ok.v+` —
`+Theorem identity : forall (A : Prop), A -> A+`.
* `+proofs/coq/admitted_stub.v+` — `+Theorem unproven+` left
`+Admitted.+`, intentionally — fixture for the axiom-scanner CI path.
* `+proofs/lean/trivial_ok.lean+` —
...
scaffold; see estate memo `+echidna_src_abi_namespace_intentional+`) and
out of scope.

=== Classification

All four files are *CI dogfood fixtures*, not proof obligations. Two are
trivial L4 identity proofs; two are deliberately incomplete to exercise
the axiom-tracker / Regulator-mode detection of `+Admitted.+` and
`+sorry+` respectively.

[width="100%",cols="33%,31%,36%",options="header",]
|===
|Layer |Count |Status
|L1 (regions) |0 |n/a — echidnabot is a CI bot, not a typed-substrate
project
...
*None.* Echidnabot’s proof surface is dogfood for its own proof-aware CI
runners; there is no content that could reuse or extend echo-types’ echo
/ loss-taxonomy / residue / image- factorisation / canonical-identity
stack.
...
* Echo-types → echidnabot: not owed (no proof-relevant content flows
back upstream).
