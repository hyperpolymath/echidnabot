...
* **GitHub** — Check Runs, PR comments, issues (full support)
* **GitLab** — Commit statuses, MR notes, issues (full support)
* **Bitbucket** — Build statuses, PR comments, issues (full support)
* **Codeberg** — Planned
...
💡 Suggested tactics:
   • Try `induction xs` to break down the list structure
   • Consider `rewrite app_assoc` if available
   • Check if `simpl` simplifies the goal
----
