...
*Overall Completion:* ~100% feature-complete; production-hardening tail in flight +
*Test Suite:* 184 tests passing (137 lib + 17 lifecycle + 32 integration + 12 property + 15 seam + 8 smoke)

toc::[]

NOTE: Older sections of this document (and the README) cite 129 tests.
That figure was pre-Package-7b; the canonical current count is 184 per
`.machine_readable/descriptiles/STATE.a2ml` (last-updated `2026-04-26`). Treat 184
as authoritative.
...
=== Double-Loop Feedback (100%)

* [x] `record_tactic_outcome` write path wired into scheduler loop after every job
* [x] `CorpusDelta` write path wired (gated by `config.corpus.enabled`)
* [x] GraphQL `recordTacticOutcome` mutation for LLM agent annotation
* [x] GraphQL `tacticOutcomes` / `tacticOutcomesByTactic` queries
* [x] `scheduler.running_count()` and `queue_depth()` lock-free accessors for `/metrics`

=== ProverKind Slug Migration (113-prover support) (100%)

* [x] Replaced 12-variant enum with `ProverSlug(String)` newtype + type alias
* [x] Classic 12 provers retained; arbitrary new provers via `ProverKind::new("slug")`
* [x] All call sites updated to use `&ProverKind` borrows or `.clone()`
* [x] `prover_key()` fixed (was `format!("{:?}")` producing `ProverSlug("coq")`)
* [x] 86 tests passing after migration

=== Automated Tests (165 tests)

* [x] 32 integration tests (webhook signatures, ECHIDNA client, bot modes, job lifecycle, database models, container executor, circuit breaker, scheduler, tactic outcome roundtrip, rate limiter)
* [x] 133 unit tests across modules (container, modes, retry, limiter, confidence, axiom tracker, solver integrity, result formatter, fleet, dispatcher, webhooks, job queue, rate_limit)
...
* [x] Wire trust bridge into scheduler loop (confidence + axiom reports in job results)
* [x] Observability: Prometheus metrics endpoint (`/metrics`)
* [x] Rate limiting for webhook endpoints (per-IP sliding window, `[server] rate_limit_rpm`)
